...
def calculate_savings_after_raise(monthly_savings: float, semi_annual_raise: float) -> float:
    """ Assumes monthly_savings float, semi_annual_raise float (percentage as 
            decimal number)
        Returns monthly_savings after raise float """
    return monthly_savings + monthly_savings * semi_annual_raise
...
def main(): 
    ...
    semi_annual_raise = get_user_input('Enter the semi­ annual raise, as a decimal (example 0.03 for 3%): ', 3)
    ...
    while current_savings < dest_amount:
        ...
        months += 1
        if months % 6 == 0: 
            monthly_savings = calculate_savings_after_raise(monthly_savings, semi_annual_raise)
